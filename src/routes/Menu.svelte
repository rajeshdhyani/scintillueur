...
<nav class="nav">
	...
	<ul class="submenu" class:menuExpanded>
		{#each nav as item}
			<li class:active={$page.route.id === item.path}>
				<MenuLink href={item.path} on:click={clickHandler}>{item.title}</MenuLink>
			</li>
		{/each}
	</ul>
</nav>
